...
//         }
   ...
   public Food getAvailableFoodInformation(Long id) {
      EntityManager em = null;
      try {
         em = EntityManagerBuilder.buildEntityManager();
         Query query = em.createNamedQuery(
                 Food.QUERY_GET_AVAILABLE_FOOD_INFORMATION);
         query.setParameter(1, id);
         List<Food> foodList = query.getResultList();
         Iterator<Food> foodIterator = foodList.iterator();
         if (foodIterator.hasNext()) {
   ...
   public Food getAvailableFoodInformation(Food food) {
      return this.getAvailableFoodInformation(food.getId());
   }
   ...
   public List<Food> getAvailableFoodListFromFoodCategory(
      ...
      try {
         em = EntityManagerBuilder.buildEntityManager();
         Query query = em.createNamedQuery(
                 Food.QUERY_GET_ALL_AVAILABLE_FOOD_FROM_FOOD_CATEGORY);
         FoodCategory category = this.getFoodCategory(foodCategory);
         if (category == null) {
            // Return an empty list.
            return new LinkedList();
   ...
   public List<Food> searchAvailableFoodByName(String name) {
      EntityManager em = null;
      try {
         em = EntityManagerBuilder.buildEntityManager();
         Query query = em.createNamedQuery(
                 Food.QUERY_SEARCH_AVAILABLE_FOOD_INFORMATION_BY_NAME);
         name = "%" + name.replaceAll("%", "") + "%";
         query.setParameter(1, name);
         return query.getResultList();
      } finally {
   ...
   public List<Food> searchAvailableFoodByCode(String code) {
      EntityManager em = null;
      try {
         em = EntityManagerBuilder.buildEntityManager();
         Query query = em.createNamedQuery(
                 Food.QUERY_SEARCH_AVAILABLE_FOOD_INFORMATION_BY_CODE);
         code = code.replaceAll("%", "") + "%";
         query.setParameter(1, code);
         return query.getResultList();
      } finally {
   ...
   public List<Food> searchAvailableFoodByTag(String tag) {
      EntityManager em = null;
      try {
         em = EntityManagerBuilder.buildEntityManager();
         Query query = em.createNamedQuery(
                 Food.QUERY_SEARCH_AVAILABLE_FOOD_INFORMATION_BY_TAG);
         tag = "%" + tag.replaceAll("%", "") + "%";
         query.setParameter(1, tag);
         return query.getResultList();
      } finally {
   ...
   public List<Food> searchAvailableFoodByKeyword(String keyword) {

      List<Food> foodList = new LinkedList();

      List<Food> foodListByName = this.searchAvailableFoodByName(keyword);
      if (foodListByName != null) {
         foodList.addAll(foodListByName);
      }

      List<Food> foodListByTag = this.searchAvailableFoodByTag(keyword);
      if (foodListByTag != null) {
         foodList.addAll(foodListByTag);
      }
